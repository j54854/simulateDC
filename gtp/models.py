...
# TODO
# ログをcsvに書き出すメソッドを追加
# ディスパッチングメソッドをConfigに移して外からオーバーライドできるようにする
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    ...
    FAILURE_LIMIT = 3  # 許容される荷卸し失敗回数（これを超えると置場に戻される）
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def release_tjob(self, tjob):
        ...
        if self.env.config.CONSOLE_OUT:
    ...
    def release_pjob(self, station):
        ...
        for item, vol in self.reqs.items():  # 全ての要求について
            while vol > 0:  # 必要量が未達なら
                ...
                candidates = self.env.gtps.buckets[item][:self.env.config.OPENABLE]
                target = candidates[0]
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def dispatch(self, candidate_tjobs):
        # if DISPATCH_RULE == PROPOSED:  # 搬送ジョブのエシェロン在庫が少ない作業場を優先
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def __init__(self, env, upward_cvs, downward_cvs):
        ...
        self.cells = [Cell(env, self, vehicle) for vehicle in range(env.config.VEHICLE)]
        # セル（台車）をノード0から順に配置していく
        for vehicle in range(env.config.VEHICLE):
            self.cells[vehicle].attach(self.nodes[env.config.VEHICLE -vehicle -1])
        # 上流コンベヤと接続
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def move_ahead(self, env, vehicle):  # 次のセルに進む
        ...
        car_ahead = self.cells[(vehicle -1 +self.env.config.VEHICLE) %self.env.config.VEHICLE]  # 先行台車
        car_behind = self.cells[(vehicle +1) %self.env.config.VEHICLE]  # 後続台車
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def handle_failure(self, env, vehicle, failure_count):  # 荷卸し失敗の処理プロセス
        if failure_count > env.config.FAILURE_LIMIT:
            this_car = self.cells[vehicle]
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def __init__(self, env):
        self.env = env
        self.pjobs = [PJob(env, idx) for idx in range(env.config.PJOB)]
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def dispatch(self, waiting_pjobs, last_picker):  # ピッキングジョブの投入ルール
        ...
        # if RELEASE_RULE == PROPOSSED:  # 提案手法は搬送ジョブ数最小の作業場を選択
        #     unpicked = [
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def operate(self, env):  # ピッキングジョブ投入のプロセス関数
        ...
        while len(waiting_pjobs) > 0:  # 投入するピッキングジョブがなくなるまで
            if len(self.unpicked_tjobs) >= env.config.RELEASABLE:  # 未作業の搬送ジョブ数の上限チェック
                yield env.tjob_picked  # ピッキング完了を待機
            # if len(self.unrestored_tjobs) >= env.config.RELEASABLE:  # 未帰還の搬送ジョブ数の上限チェック
...
# ---------- * ---------- * ---------- * ---------- * ----------
def create_simulator(config=DefaultConfig):
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
