...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def get_requirements(self):  # 要求の辞書を作成
        # 所定の数のアイテム番号をランダムに決定
        keys = random.sample(range(util.ITEM), self.num)
    ...
    def release_pjob(self, station):
        ...
        for item, vol in self.reqs.items():  # 全ての要求について
            while vol > 0:  # 必要量が未達なら
                ...
                candidates = self.env.gtps.buckets[item][:util.OPENABLE]
                target = candidates[0]
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def __init__(self, env, pjob, idx, bucket, vol):
        ...
        self.vol = vol
        # 状態変数
        self.progress = util.TO_FW_SHTL  # 初期値（往路シャトル台車からの降車前）
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def get_preceding_tjobs(self):
        ...
        pjobs_ahead = [pjob for pjob in self.env.pjobs if pjob.idx < its_pjob_idx and pjob.station == its_station]
        # それらに対応する搬送ジョブを抽出
        tjobs_ahead = [tjob for pjob in pjobs_ahead for tjob in pjob.tjobs]
        # シャトルの手前なら，同じ系列，同じ階層のものに絞る
        if self.progress == util.TO_FW_SHTL:
        ...
        if self.progress == util.TO_FW_LIFT:
            tjobs_ahead = [tjob for tjob in tjobs_ahead if tjob.bucket.home_store.aisle == its_aisle and tjob.bucket.home_store.floor != its_floor]
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def proceed(self, to=None, work_time=None):  # 進捗段階を進める
        if self.progress == util.TO_PICKED:  # ピッキング作業前から進捗が進むと
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def get_goal_node_idx(self):  # ループ台車の行先ノード
        ...
        if self.progress == util.TO_FW_LOOP:  # 出庫でループに入る
            return util.AISLE_SEG[aisle][util.FORWARD]
        elif self.progress == util.ON_FW_LOOP:  # 出庫でループから出る
            return util.PICKER_SEG[picker][util.FORWARD]
        elif self.progress == util.TO_BW_LOOP:  # 入庫でループに入る
            return util.PICKER_SEG[picker][util.BACKWARD]
        elif self.progress == util.ON_BW_LOOP:  # 入庫でループから出る
            return util.AISLE_SEG[aisle][util.BACKWARD]
        else:
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def get_store_address(self):  # 置場の系列・階層・段階を返す（__repr__用）
        if self.home_store is not None:
            ...
            return 'unassigned'
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def get_position(self):  # 所在地の座標を返す（描画用）
        return self.cell.get_position()
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def undone_tjobs(self):  # 未完了の搬送ジョブリスト
        return [tjob for tjob in self.tjobs if tjob.progress < util.DONE]
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def picked_tjobs(self):  # ピッキング済みの搬送ジョブリスト
        return [tjob for tjob in self.tjobs if tjob.progress > util.TO_PICKED and not tjob.retry_needed]
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def priority(self):  # 搬送ジョブ割付けの優先度（小さいほど優先）
        ...
        priority = util.DONE *len(self.undone_tjobs())
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def pick(self, tjob):  # ピッキング処理と入庫の必要性確認
        ...
        if self.get_fullness() <= 0:  # バケットが空なら
            ...
            tjob.proceed(to=util.DONE)  # 搬送ジョブはここで完了（util.TO_BW_LOOP > util.DONE）
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def draw(self, screen):  # 描画
        ...
        bucket_col = util.BUCKET_COL[self.item %7]
        pg.draw.circle(screen, [col *self.item /util.ITEM *223 for col in bucket_col], pos, util.DX *0.4)
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def __init__(self, env, unit, idx):
        ...
        if self.unit.__class__.__name__ == 'Loop':  # ループの台車なら
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def __repr__(self):
        if self.unit.__class__.__name__ == 'Loop':
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def draw(self, screen):  # 描画
        pos = self.get_position()
        screen.fill(util.WHITE, (pos[0] -util.DX *0.45, pos[1] -util.DY *0.45, util.DX *0.9, util.DY *0.9))
        pg.draw.rect(screen, util.BLACK, (pos[0] -util.DX *0.45, pos[1] -util.DY *0.45, util.DX *0.9, util.DY *0.9), 1)
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def __init__(self, env, unit, idx, pos):
        self.env = env
        self.unit = unit  # ユニットへのポインタ
        self.idx = idx  # ノード番号
        self.pos = pos  # 所在地の座標 (x,y)
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def draw(self, screen):  # 描画
        pg.draw.circle(screen, util.BLACK, self.pos, 2)
        if self.to is not None:
            pg.draw.aaline(screen, util.BLACK, self.pos, self.to.pos)
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def echelon_filter(self, tjobs, base_progress):  # エシェロン在庫数最小の作業場に向かう搬送ジョブに絞る
        ...
        for count in sorted(set(echelon_count)):  # 重複を削除して昇順に在庫数を取り出す
            ...
            target_pickers = [picker for picker in range(util.PICKER) if echelon_count[picker] == count]
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def __init__(self, env, aisle, floor, row):
        ...
        self.floor = floor
        self.row = row
        self.nodes.append(Node(env, self, 0, util.get_store_node_position(aisle, floor, row)))
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def __init__(self, env, aisle, floor, stores, conveyors):
        ...
        self.aisle = aisle
        self.floor = floor
        self.nodes = [  # レールノードの作成
            Node(env, self, row, util.get_shuttle_node_position(aisle, floor, row))
            for row in range(util.ROW +1)  # 最後は台車の待機ノード
            ]
        # セル（台車）を作成し，待機ノードに配置
        self.cells.append(Cell(env, self, 0))
        self.cells[0].attach(self.nodes[util.ROW])
        # レールノードと置場ノードを相互に接続
        for row in range(util.ROW):
            self.nodes[row].connect_to(stores[row].nodes[0])
            stores[row].nodes[0].connect_to(self.nodes[row])
        # 待機ノードを出庫コンベヤに接続
        self.nodes[util.ROW].connect_to(conveyors[util.FORWARD].nodes[0])
        # 入庫コンベヤに待機ノードを接続
        conveyors[util.BACKWARD].nodes[-1].connect_to(self.nodes[util.ROW])
        # 出庫ジョブ到着イベントを初期化
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def dispatch(self, candidate_tjobs):
        if util.DISPATCH_RULE == util.PROPOSED:  # 搬送ジョブのエシェロン在庫が少ない作業場を優先
            candidate_tjobs = self.echelon_filter(candidate_tjobs, util.TO_FW_LIFT)
    ...
    def operate(self, env):  # シャトルのプロセス関数
        ...
        while True:
            if current_tjob is None:  # 待機中なら
                ...
                bw_tjobs = [  # 入庫ジョブリスト
                    tjob for tjob in self.tjobs if tjob.progress == util.TO_BW_SHTL
                    ]
                if len(fw_tjobs) +len(bw_tjobs) <= 0:  # 着手可能な搬送ジョブがなければ
                    # 出庫ジョブの発生，もしくは，入庫ジョブの発生を待ち受ける
                    yield (self.tjob_registered | self.nodes[util.ROW].ot.cell.bucket_receipt)
                else:
                    # 出庫ジョブがない，もしくは，前回出庫で入庫ジョブがあれば
                    if len(fw_tjobs) <= 0 or (direction == util.FORWARD and len(bw_tjobs) > 0):
                        current_tjob = bw_tjobs[0]  # 先頭の入庫ジョブを選択（コンベヤの先頭のもの）
                        direction = util.BACKWARD  # 搬送方向を入庫に設定
                    else:
                        current_tjob = self.dispatch(fw_tjobs)  # 出庫ジョブのディスパッチング
                        direction = util.FORWARD  # 搬送方向を出庫に設定
                ...
                if self.cells[0].is_open():  # 空車なら
                    ...
                    if self.cells[0].node.idx < util.ROW:  # 待機ノードに未到着なら
                        yield env.timeout(util.MT_SHTL)  # 移動時間分の時間遅れ
                        ...
                        current_tjob.proceed()  # 搬送ジョブの進捗を進める（util.TO_FW_SHTL > util.TO_FW_LIFT）
                        current_tjob = None  # 実行中の搬送ジョブを削除
            elif current_tjob.progress == util.TO_BW_SHTL:  # 入庫ジョブの処理中なら
                if not self.cells[0].is_open():  # 運搬中なら
                    ...
                    if self.cells[0].node.idx == util.ROW:  # 待機ノードにいる（出発前）なら
                        yield env.timeout(util.LT_SHTL)  # 積込み時間分の時間遅れ
                        ...
                        if self.cells[0].node.idx == util.ROW:  # 待機ノードに帰還したなら
                            current_tjob.proceed()  # 搬送ジョブの進捗を進める（util.TO_BW_SHTL > util.DONE）
                            current_tjob = None  # 実行中の搬送ジョブを削除
            else:
                assert False, 'Shuttle job has inconsistent progress status'
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def draw(self, screen):  # 描画
        pg.draw.aaline(screen, util.BLACK, self.nodes[0].pos, self.nodes[-2].pos)
        pg.draw.aaline(screen, util.BLACK, self.nodes[-2].pos, self.nodes[-1].pos)
        super().draw(screen)
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def __init__(self, env, stage, dim0, dim1, direction):
        ...
        self.nodes = [
            Node(env, self, seg, util.get_conveyor_node_position(stage, dim0, dim1, direction, seg))
            for seg in range(util.CONV_LEN[stage])
        ]
        self.connect_nodes()
        # セルを作成し，ノードに対応付ける
        self.cells = [Cell(env, self, seg) for seg in range(util.CONV_LEN[stage])]
        for seg in range(util.CONV_LEN[stage]):
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def operate(self, env, seg):  # コンベヤ各セルのプロセス関数
        ...
        while True:
            ...
            yield env.timeout(util.MT_CONV)  # 移動時間分の時間遅れ
            ...
            if next_cell == self.cells[-1]:
                ...
                if util.PRINT_CONSOLE:
                    if self.direction == util.FORWARD:
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def draw(self, screen):  # 描画
        pg.draw.aaline(screen, util.BLACK, self.nodes[0].pos, self.nodes[-1].pos)
        super().draw(screen)
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def __init__(self, env, aisle, direction, upward_cvs, downward_cvs):
        ...
        self.direction = direction
        self.nodes = [
            Node(env, self, floor, util.get_lift_node_position(aisle, floor, direction))
            for floor in range(util.FLOOR)
            ]
        # セル（台車）を作成し，0階に配置
        self.cells.append(Cell(env, self, 0))
        self.cells[0].attach(self.nodes[0])
        ...
        for floor in range(util.FLOOR):
            if direction == util.FORWARD:
                upward_cvs[floor][util.FORWARD].nodes[-1].connect_to(self.nodes[floor])
            else:
                self.nodes[floor].connect_to(upward_cvs[floor][util.BACKWARD].nodes[0])
        # 下流コンベヤと接続
        if direction == util.FORWARD:
            self.nodes[0].connect_to(downward_cvs[util.FORWARD].nodes[0])
        else:
            downward_cvs[util.BACKWARD].nodes[-1].connect_to(self.nodes[0])
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def dispatch(self, candidate_tjobs):
        if util.DISPATCH_RULE == util.PROPOSED:  # 搬送ジョブのエシェロン在庫が少ない作業場を優先
            candidate_tjobs = self.echelon_filter(candidate_tjobs, util.TO_FW_LOOP)
        pjob_idx = [tjob.pjob.idx for tjob in candidate_tjobs]  # ピッキングジョブのインデックス
        # が最小のものだけに絞る
        candidate_tjobs = [tjob for tjob in candidate_tjobs if tjob.pjob.idx == min(pjob_idx)]
        return random.choice(candidate_tjobs)
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def operate(self, env):  # リフトのプロセス関数
        current_tjob = None  # 実行中の搬送ジョブ
        if self.direction == util.FORWARD:  # 出庫リフト
            while True:
                if current_tjob is None:  # 待機中なら
                    candidate_tjobs = self.precedence_filter(self.tjobs)  # 先行関係制約でフィルタリング
                    if len(candidate_tjobs) <= 0:  # 着手可能な搬送ジョブがなければ
                        yield env.any_of([self.nodes[floor].ot.cell.bucket_receipt for floor in range(util.FLOOR)])
                    else:
                        current_tjob = self.dispatch(candidate_tjobs)  # 搬送ジョブのディスパッチング
                        assert current_tjob.progress == util.TO_FW_LIFT, 'Wrong progress at forward lift'
                        self.tjobs.remove(current_tjob)  # 選択した搬送ジョブをリストから削除
                        if util.PRINT_CONSOLE:
                    ...
                    if self.cells[0].is_open():  # 空車なら
                        if self.cells[0].node.idx < current_tjob.bucket.home_store.floor:  # 階層に未到着なら
                            yield env.timeout(util.MT_LIFT)  # 移動時間分の時間遅れ
                            self.cells[0].move_up()  # 上階のセルに移動
                        else:  # 階層に到着したら
                            yield env.timeout(util.LT_LIFT)  # 積替え時間分の時間遅れ
                            self.cells[0].node.ot.cell.transfer()  # バケットを受け取る
                            if util.PRINT_CONSOLE:
            ...
            while True:
                if current_tjob is None:  # 待機中なら
                    if len(self.tjobs) <= 0:  # 着手可能な搬送ジョブがなければ
                        ...
                        assert current_tjob.progress == util.TO_BW_LIFT, 'Wrong progress at backward lift'
                        self.tjobs.remove(current_tjob)  # 選択した搬送ジョブをリストから削除
                        if util.PRINT_CONSOLE:
                    ...
                    if self.cells[0].is_open():  # 空車なら
                        ...
                        if self.cells[0].node.idx < current_tjob.bucket.home_store.floor:  # 階層に未到達なら
                            ...
                            current_tjob.proceed()  # 搬送ジョブの進捗を進める（util.TO_BW_LIFT > util.TO_BW_SHTL）
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def __init__(self, env, upward_cvs, downward_cvs):
        super().__init__(env)
        self.nodes = [Node(env, self, seg, util.get_loop_node_position(seg)) for seg in range(util.LOOP)]
        self.cells = [Cell(env, self, vehicle) for vehicle in range(util.VEHICLE)]
        # セル（台車）をノード0から順に配置していく
        for vehicle in range(util.VEHICLE):
            self.cells[vehicle].attach(self.nodes[util.VEHICLE -vehicle -1])
        # 上流コンベヤと接続
        for aisle in range(util.AISLE):
            upward_cvs[aisle][0][util.FORWARD].nodes[-1].connect_to(self.nodes[util.AISLE_SEG[aisle][util.FORWARD]])
            self.nodes[util.AISLE_SEG[aisle][util.BACKWARD]].connect_to(upward_cvs[aisle][0][util.BACKWARD].nodes[0])
        # 下流コンベヤと接続
        for picker in range(util.PICKER):
            self.nodes[util.PICKER_SEG[picker][util.FORWARD]].connect_to(downward_cvs[picker][0][util.FORWARD].nodes[0])
            downward_cvs[picker][0][util.BACKWARD].nodes[-1].connect_to(self.nodes[util.PICKER_SEG[picker][util.BACKWARD]])
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    ...
    def get_nearest_car(self, cars, destination_node):
        ...
        if min(dist) <= 0:  # 眼の前のセルに台車があれば
            ...
            if candidate_car.goal is None:  # 待機状態なら
                ...
                dist[dist.index(min(dist))] = util.LOOP  # 移動後にその台車は周回遅れになる
        nearest_idx = dist.index(min(dist))  # 距離が最短の台車のインデックス
        return cars[nearest_idx]
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def dispatch(self, candidate_fw_tjobs):
        if util.DISPATCH_RULE == util.PROPOSED:  # 搬送ジョブのエシェロン在庫が少ない作業場を優先
            candidate_fw_tjobs = self.echelon_filter(candidate_fw_tjobs, util.ON_FW_LOOP)
        for tjob in self.tjobs:  # 前から順に見ていく
            if tjob.progress == util.TO_BW_LOOP:  # 入庫ジョブなら
                return tjob  # それを実行
            else:  # 出庫ジョブなら
    ...
    def operate(self, env):  # 搬送ジョブの台車への割付け
        while True:
            ...
            if len(self.tjobs) > 0 and len(free_cars) > 0:  # どちらも存在するなら
                fw_tjobs = [tjob for tjob in self.tjobs if tjob.progress == util.TO_FW_LOOP]  # 出庫ジョブリスト
                fw_tjobs = self.precedence_filter(fw_tjobs)  # 先行関係制約でフィルタリング
                ...
                if util.PRINT_CONSOLE:
                    print(f'Loop assignes {'forward' if current_tjob.progress == util.TO_FW_LOOP else 'backward'} Tjob {current_tjob.pjob.idx}-{current_tjob.idx} to car {the_car.idx}')
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def move_ahead(self, env, vehicle):  # 次のセルに進む
        ...
        car_ahead = self.cells[(vehicle -1 +util.VEHICLE) %util.VEHICLE]  # 先行台車
        car_behind = self.cells[(vehicle +1) %util.VEHICLE]  # 後続台車
        ...
        yield env.timeout(util.MT_LOOP)  # 移動時間分の時間遅れ
        this_car.move_to(next_node)
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def handle_failure(self, env, vehicle, failure_count):  # 荷卸し失敗の処理プロセス
        if failure_count > util.FAILURE_LIMIT:
            this_car = self.cells[vehicle]
            this_car.tjob.retry_needed = True
            this_car.tjob.proceed(util.ON_BW_LOOP)  # 進捗を出庫側から入庫側に変更
        ...
        while True:
            if this_car.tjob is None:  # 搬送ジョブが割り付けられていないなら
                ...
                if this_car.goal != this_car.node:  # 行き先への移動の途上なら
                    ...
                    if this_car.tjob.progress in [util.TO_FW_LOOP, util.TO_BW_LOOP]:  # 荷積み前
                        yield env.timeout(util.LT_LOOP)  # 積込み時間分の時間遅れ
                        this_car.node.ot.cell.transfer()  # バケットを受け取る
                        if util.PRINT_CONSOLE:
                            print(f'Car {this_car.idx} picks up at cell {this_car.node.idx} {'forward' if this_car.tjob.progress == util.TO_FW_LOOP else 'backward'} Tjob {this_car.tjob.pjob.idx}-{this_car.tjob.idx} at {env.now:.2f}')
                        this_car.tjob.proceed()  # 搬送ジョブの進捗を進める
                        this_car.assign_goal(self.nodes[this_car.tjob.get_goal_node_idx()])  # 行き先を更新
                    elif this_car.tjob.progress in [util.ON_FW_LOOP, util.ON_BW_LOOP]:  # 荷降ろし前
                        if this_car.tjob.progress == util.ON_FW_LOOP and this_car.tjob.late_preceding_tjob_count() > 0:  # 先行関係制約が満たされていないなら（まだ降ろせない）
                            ...
                            if not this_car.node.to.cell.is_open():  # 荷降ろしがブロックされたら
                                ...
                                if util.PRINT_CONSOLE:
                                    print(f'Car {this_car.idx} drops at cell {this_car.node.idx} {'forward' if this_car.tjob.progress == util.ON_FW_LOOP else 'backward'} Tjob {this_car.tjob.pjob.idx}-{this_car.tjob.idx} at {env.now:.2f}')
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def draw(self, screen):  # 描画
        prev = self.nodes[-1]
        for node in self.nodes:
            pg.draw.aaline(screen, util.BLACK, prev.pos, node.pos)
            prev = node
        super().draw(screen)
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def __init__(self, env, picker, conveyors):
        super().__init__(env)
        self.picker = picker
        self.nodes.append(Node(env, self, 0, util.get_station_node_position(picker)))
        self.cells.append(Cell(env, self, 0))
        # ノードとセルを対応付ける
        self.cells[0].attach(self.nodes[0])
        # 出庫コンベヤを作業場に接続
        conveyors[util.FORWARD].nodes[-1].connect_to(self.nodes[0])
        # 作業場を入庫コンベヤに接続
        self.nodes[0].connect_to(conveyors[util.BACKWARD].nodes[0])
        # 稼働率計算用
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def get_utilization(self):  # 稼働率の計算
        utilized = self.total_tjobs *util.MT_CONV *2 +self.total_work_time  # 搬出・搬入時間を加算
        return utilized /self.makespan
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def get_work_time(self):
        work_time = random.choices(util.T_PICK, weights=util.P_PICK)[0]
        ...
        while True:
            if current_tjob is None:  # 待機中なら
                if len(self.tjobs) <= 0:  # 着手可能な搬送ジョブがなければ
                    ...
                    assert current_tjob.progress == util.TO_PICKED, 'Wrong progress at station'
                ...
                if self.cells[0].is_open():  # 作業場が空いているなら
                    ...
                    # 搬送ジョブの進捗を進め（util.TO_PICKED -> util.TO_BW_LOOP），作業時間を記録
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def __init__(self, env):
        ...
        self.stores = [[[  # 置場ユニットの作成
            Store(env, aisle, floor, row) for row in range(util.ROW)
            ] for floor in range(util.FLOOR)] for aisle in range(util.AISLE)]
        ...
        self.conveyors = [[[[  # コンベヤユニットの作成
            ...
            ] for dim1 in range(util.CONV_DIM[1][stage])
            ] for dim0 in range(util.CONV_DIM[0][stage])] for stage in range(3)]
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def get_echelon_count(self, base_progress):  # ピッキング作業場までのエシェロン在庫量を返す
        echelons = [[tjob for tjob in self.unpicked_tjobs
            if tjob.pjob.station.picker == picker and base_progress <= tjob.progress <= util.TO_PICKED
            ] for picker in range(util.PICKER)]
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def dispatch(self, waiting_pjobs, last_picker):  # ピッキングジョブの投入ルール
        ...
        if util.RELEASE_RULE == util.DEFAULT:  # デフォルトはラウンドロビン
            next_picker = (last_picker +1) %util.PICKER
        else:  # 提案手法は搬送ジョブ数最小の作業場を選択
            unpicked = [
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def operate(self, env):  # ピッキングジョブ投入のプロセス関数
        ...
        waiting_pjobs = env.pjobs.copy()  # 未投入ピッキングジョブのリストを初期化
        last_picker = -1  # 割り付けた作業場の番号を初期化（ラウンドロビンルールで使用）
        while len(waiting_pjobs) > 0:  # 投入するピッキングジョブがなくなるまで
            if len(self.unpicked_tjobs) >= util.RELEASABLE_UNPICKED:  # 未作業の搬送ジョブ数の上限チェック
                yield env.tjob_picked  # ピッキング完了を待機
            if len(self.unrestored_tjobs) >= util.RELEASABLE_UNRESTORED:  # 未帰還の搬送ジョブ数の上限チェック
...
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    def draw(self, screen):  # 描画
        for aisle in range(util.AISLE):
            for floor in range(util.FLOOR):
                for row in range(util.ROW):
                    self.stores[aisle][floor][row].draw(screen)
                self.shuttles[aisle][floor].draw(screen)
                self.conveyors[0][aisle][floor][0].draw(screen)
                self.conveyors[0][aisle][floor][1].draw(screen)
            self.lifts[aisle][0].draw(screen)
            self.lifts[aisle][1].draw(screen)
            self.conveyors[1][aisle][0][0].draw(screen)
            self.conveyors[1][aisle][0][1].draw(screen)
        self.loop.draw(screen)
        for picker in range(util.PICKER):
            self.conveyors[2][picker][0][0].draw(screen)
            self.conveyors[2][picker][0][1].draw(screen)
            self.stations[picker].draw(screen)

# ---------- * ---------- * ---------- * ---------- * ----------
def set_simpy():
# ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
    env = simpy.Environment()
    env.pjobs = [PJob(env, idx) for idx in range(util.PJOB)]  # ピッキングジョブのリストをenvに置く
    env.gtps = GTPSystem(env)
    env.gameover = env.event()

    env.process(env.gtps.operate(env))
    for aisle in range(util.AISLE):
        for floor in range(util.FLOOR):
            env.process(env.gtps.shuttles[aisle][floor].operate(env))
    for aisle in range(util.AISLE):
        for direction in range(2):
            env.process(env.gtps.lifts[aisle][direction].operate(env))
    env.process(env.gtps.loop.operate(env))
    for vehicle in range(util.VEHICLE):
        env.process(env.gtps.loop.drive(env, vehicle))
    for stage in range(3):
        for dim0 in range(util.CONV_DIM[0][stage]):
            for dim1 in range(util.CONV_DIM[1][stage]):
                for direction in range(2):
                    for seg in range(util.CONV_LEN[stage] -1):
                        env.process(env.gtps.conveyors[stage][dim0][dim1][direction].operate(env, seg))
    for picker in range(util.PICKER):
        env.process(env.gtps.stations[picker].operate(env))
    return env
